...
class Main extends PluginBase implements Listener{
 ...
 NightMC > Sell Plugin Enable | Arthur: JackMD 
			...
			if(!($sender instanceof Player)){
				...
				if($sender->hasPermission("sell") || $sender->hasPermission("sell.hand") || $sender->hasPermission("sell.all")){
					...
					if(isset($args[0]) && strtolower($args[0]) == "hand"){
						...
						$sender->sendMessage("§l§f❖ §eĐã bán được§a $price xu");
	...
	$sender->sendMessage("§l§f❖ §eĐã bán được§a $priceall xu");
					}elseif(isset($args[0]) && strtolower($args[0]) == "about"){
						$sender->sendMessage(TF::RED . TF::BOLD . "(!) " . TF::RESET . TF::GRAY . "404 ×.× > Nothing here :(");
					}else{
						$sender->sendMessage("§l§f❖§6 /sell all:§e Bán tất cả đồ trong túi");
$sender->sendMessage("§l§f❖§6 /sell hand:§e Bán đồ trên tay");
						return true;
					}
				}else{
