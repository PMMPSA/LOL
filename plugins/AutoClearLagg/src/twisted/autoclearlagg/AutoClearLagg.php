...
class AutoClearLagg extends PluginBase{
    ...
    public function onEnable() : void{
        ...
        if(!is_numeric($config["seconds"] ?? 14400)){
        ...
        $this->messages = [
            self::LANG_TIME_LEFT => $messages[self::LANG_TIME_LEFT] ?? "§f[ §b§lSκʏ§aL§cO§aL§f ] §fDọn item rơi trên mặt đất sau§9 {SECONDS} nữa",
            self::LANG_ENTITIES_CLEARED => $messages[self::LANG_ENTITIES_CLEARED] ?? "§f[ §b§lSκʏ§aL§cO§aL§f ] §7Dọn thành công§f {COUNT} item"
        ];
        ...
        $this->broadcastTimes = $config["times"] ?? [60, 30, 15, 3, 2, 1];
