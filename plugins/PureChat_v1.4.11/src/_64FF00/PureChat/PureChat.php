...
class PureChat extends PluginBase
{
    ...
    */
     ...
     * @return mixed
     */
public function getPolice(Player $player, $levelName = null)
    {
        if($levelName === null)
        {
            return $this->purePerms->getUserDataMgr()->getNode($player, "pol");
        }
        else
        {
            $worldData = $this->purePerms->getUserDataMgr()->getWorldData($player, $levelName);

            if(!isset($worldData["pol"]) || $worldData["pol"] === null)
                return "";

            return $worldData["pol"];
        }
    }
...
$string = str_replace("{police_tag}", $this->getPolice($player, $levelName), $string);
$string = str_replace("{l}", "\n", $string);
...
public function setPolice($pol, Player $player, $levelName = null)
    {
        if($levelName === null)
        {
            $this->purePerms->getUserDataMgr()->setNode($player, "pol", $pol);
        }
        else
        {
            $worldData = $this->purePerms->getUserDataMgr()->getWorldData($player, $levelName);
