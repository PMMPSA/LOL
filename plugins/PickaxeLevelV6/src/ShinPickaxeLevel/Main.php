...
class Main extends PluginBase implements Listener{
	...
	public function onItemHeld(PlayerItemHeldEvent $ev){
		...
		if ($it[0] == "§l§e⚡") {
			if ($damage > 780) {
				...
				$p->sendMessage("§l§6[§aʟ§cᴏ§aʟ§6]§e Cúp đã được sửa chữa miễn phí!");
			}
			$popup = "   §l§e⚡§6 ᴘɪᴄᴋᴀxᴇ §7│ §bʟᴏʟ§c §e⚡\n" . "§e♜§b Kinh Nghiệm:§a " . $exp ."§l§6 /§c ".$next. "§7 |§6 Cấp Cúp: §a" . $level;
			$p->sendPopup($popup);
		}	
	...
	public function onBreak(BlockBreakEvent $ev){
		...
		if($pas[0] == "§l§e⚡"){
			if(strpos($icn, $name)  == false){
				...
				$p->sendMessage("§l§6[§aʟ§cᴏ§aʟ§6]§e Vật phẩm không phải của bạn!");
				}
		}
		    ...
		    if($pas[0] == "§l§e⚡"){
				...
				if($this->getExp($p) >= $this->getNextExp($p)){
					...
					$this->getServer()->broadcastMessage("§l§6[§aʟ§cᴏ§aʟ§6]§r§6 Cúp của người chơi§l§c ".$p->getName()."§r§6 Vừa lên cấp:§b ".$this->getLevel($p));
			...
			if($cs > 1){
				...
				if($add > 1){
					...
					$p->sendMessage("§l§6[§aʟ§cᴏ§aʟ§6]§a Bạn đã nhận được§d $add xu §btừ chuyển sinh cấp§c $cs");
				}
			}
	}
	...
	public function onCommand(CommandSender $sender, Command $cmd,string $label, array $args) : bool {
         if($cmd->getName() == "givecup"){
			 if($sender->isOp() || $sender->getName() == "NoobPvP2k3"){
                if(!isset($args[0])){
                    $sender->sendMessage("§l§6[§aʟ§cᴏ§aʟ§6]§a /givecup <player>");
                    return true;
                }else{
                    ...
                    if(!$player == null){
                        if($player->isOnline()) {
                            ...
                            $inv->addItem($cup);
                            $player->sendMessage("§l§6[§aʟ§cᴏ§aʟ§6]§e Cúp §l§6ʟᴏʟ ...");		
                 ...
                 $sender->sendMessage("§l§6[§aʟ§cᴏ§aʟ§6]§b You don't permission use command");
                 return true;
             }
			...
			if($sender->isOp()){
			   $sender->sendMessage("§l§6[§aʟ§cᴏ§aʟ§6]§c Bạn là op đừng chiếm top của mem!");
			   return true;
			}
			if(!($tien >= $amount*$solan)){
				$sender->sendMessage("§l§6[§aʟ§cᴏ§aʟ§6]§e Bạn không có đủ tiền để§a chuyển sinh! §eBạn cần§c ".$amount*$solan." xu");
			}else{
				$sender->sendMessage("§l§6[§aʟ§cᴏ§aʟ§6]§e Bạn đã §achuyển sinh §ethành công! + 1 lần chuyển sinh");
	...
	public function getNamePickaxe(Player $player){
		return "§l§e⚡ §6ʟᴏʟ §bᴘɪᴄᴋᴀxᴇ §7[§cLevel:§6 ". (string)$this->getLevel($player) ." §7]§a ".$player->getName(); 
